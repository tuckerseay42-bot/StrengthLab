...
CREATE TABLE IF NOT EXISTS public.rack_athlete_exercise_overrides (
...
CREATE POLICY "Org members manage rack exercise overrides"
...
CREATE INDEX IF NOT EXISTS idx_rack_overrides_session_athlete
  ON public.rack_athlete_exercise_overrides (rack_session_id, athlete_id);

CREATE TRIGGER trg_rack_overrides_touch
  BEFORE UPDATE ON public.rack_athlete_exercise_overrides
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

-- Update log-to-lift trigger to honor per-athlete substitutions
...
BEGIN
  ...
  SELECT substitute_exercise_name INTO _override
  ...
  IF _override.substitute_exercise_name IS NOT NULL THEN
    _ex_name := _override.substitute_exercise_name;
  ELSE
