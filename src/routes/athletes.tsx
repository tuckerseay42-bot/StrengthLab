...
function AthletesPage() {
  ...
  const [bulkForm, setBulkForm] = useState({
    ...
    gender: "__keep", class_period: "__keep", program_id: "__keep",
  });
  ...
  const classPeriodOptions = useMemo(() => {
    ...
    for (const a of athletes) {
      const cp = (a as unknown as { class_period: string | null }).class_period;
      if (cp && cp.trim()) s.add(cp.trim());
    }
    return Array.from(s).sort();
  }, [athletes]);

  const filtered = useMemo(() => {
    const base = filterAthletes(athletes, filters, activeTeamId, teamsByAthlete);
    if (classPeriodFilter === "all") return base;
    if (classPeriodFilter === "__none") {
      return base.filter((a) => !(a as unknown as { class_period: string | null }).class_period);
    }
    return base.filter(
      (a) => (a as unknown as { class_period: string | null }).class_period === classPeriodFilter,
    );
  }, [athletes, filters, activeTeamId, teamsByAthlete, classPeriodFilter]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (editing) {
        ...
        const { error } = await supabase.from("athletes").insert({ ...payload, organization_id });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  const bulkEdit = useMutation({
    ...
    onSuccess: (n) => {
      ...
      setBulkOpen(false);
      setSelected(new Set());
      setBulkForm({ team_id: "__keep", status: "__keep", grade: "__keep", sport: "__keep", gender: "__keep", class_period: "__keep", program_id: "__keep" });
    },
    onError: (e: Error) => toast.error(toUserMessage(e)),
  });
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2 min-w-0 flex-1">
                    <Checkbox
                      ...
                      className="mt-1"
                    />
                    <Link to="/athletes/$id" params={{ id: a.id }} className="min-w-0 flex-1">
                    <div className="truncate font-semibold hover:underline">{athleteDisplayName(a)}</div>
                    <div className="mt-1 flex flex-wrap gap-1">
                      {teamMap.get(a.team_id ?? "") && <Badge variant="outline">{teamMap.get(a.team_id ?? "")!.name}</Badge>}
                      {a.sport && <Badge variant="secondary">{a.sport}</Badge>}
                      {(a as unknown as { sport_fall: string | null }).sport_fall && <Badge variant="secondary">Fall: {(a as unknown as { sport_fall: string }).sport_fall}</Badge>}
                      {(a as unknown as { sport_winter: string | null }).sport_winter && <Badge variant="secondary">Winter: {(a as unknown as { sport_winter: string }).sport_winter}</Badge>}
                      {(a as unknown as { sport_spring: string | null }).sport_spring && <Badge variant="secondary">Spring: {(a as unknown as { sport_spring: string }).sport_spring}</Badge>}
                      {a.grade && <Badge variant="outline">Grade {a.grade}</Badge>}
                      
                      {a.bodyweight && <Badge variant="outline">{fromLb(a.bodyweight, prefs.weight).toFixed(1)} {prefs.weight}</Badge>}
                      {duplicateIds.has(a.id) && <Badge variant="outline" className="border-amber-500/50 text-amber-500">Duplicate</Badge>}
                      {a.status !== "active" && <Badge variant={a.status === "injured" ? "destructive" : "secondary"}>{a.status}</Badge>}
                      {(() => {
                        const s = (a as unknown as { invite_status?: string }).invite_status ?? "not_invited";
                        if (s === "pin_set" || s === "active") return <Badge variant="outline" className="border-emerald-500/40 text-emerald-500"><Mail className="mr-1 h-3 w-3" /> PIN set</Badge>;
                        if (s === "sent" || s === "delivered" || s === "opened") return <Badge variant="outline" className="border-primary/40 text-primary"><Mail className="mr-1 h-3 w-3" /> Invite sent</Badge>;
                        if (s === "failed" || s === "expired") return <Badge variant="destructive"><Mail className="mr-1 h-3 w-3" /> Invite {s}</Badge>;
                        return null;
                      })()}
                    </div>
                  ...
                  <div className="flex shrink-0 gap-1">
                    <Link to="/athletes/$id" params={{ id: a.id }}>
                      <Button size="icon" variant="ghost" aria-label="Open card"><ExternalLink className="h-4 w-4" /></Button>
                    </Link>
                    <Button size="icon" variant="ghost" onClick={() => openEdit(a)} aria-label="Edit"><Pencil className="h-4 w-4" /></Button>
                    ...
                    <Button size="icon" variant="ghost" onClick={() => { if (confirm(`Remove ${athleteDisplayName(a)}? All their tests, lifts, and attendance will also be deleted.`)) del.mutate(a.id); }} aria-label="Delete">
                ...
                {canAssignProgram && (
                  <div className="mt-3 flex items-center gap-2 border-t pt-2">
                    <Label className="shrink-0 text-xs text-muted-foreground">Program</Label>
                    <Select
