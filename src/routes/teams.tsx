...
function TeamsPage() {
  ...
  const rosterCounts = useMemo(() => {
    ...
    athletes.forEach((a) => { if (a.team_id) bump(a.team_id, a.id); });
    athleteTeams.forEach((r) => { if (r.team_id && r.athlete_id) bump(r.team_id, r.athlete_id); });
    return m;
  ...
  const pendingCounts = useMemo(() => {
    const m = new Map<string, number>();
    registrations.filter((r) => r.status === "pending").forEach((r) => m.set(r.team_id, (m.get(r.team_id) ?? 0) + 1));
    return m;
  }, [registrations]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        name: form.name.trim(), sport: form.sport || null, season: form.season.trim() || null,
        color: form.color || null, notes: form.notes.trim() || null,
      };
      if (editing) {
    ...
    onSuccess: () => {
      ...
      setOpen(false); setEditing(null); setForm({ name: "", sport: "", season: "", color: "#F97316", notes: "" });
    },
  ...
  const regen = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["teams"] }); toast.success("QR link regenerated"); },
  });

  const archive = useMutation({
    mutationFn: async (t: Team) => {
      const { error } = await supabase.from("teams").update({ archived_at: new Date().toISOString() }).eq("id", t.id);
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["teams"] }); toast.success("Team archived"); },
  });
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{visibleTeams.length} active team{visibleTeams.length === 1 ? "" : "s"}</p>
        </div>
        <Button size="sm" onClick={() => { setEditing(null); setForm({ name: "", sport: "", season: "", color: "#F97316", notes: "" }); setOpen(true); }}>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="h-3 w-3 rounded-full" style={{ background: t.color ?? "#F97316" }} />
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="grid gap-3">
            <div><Label>Name *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Varsity Football" /></div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label>Season</Label><Input value={form.season} onChange={(e) => setForm({ ...form, season: e.target.value })} placeholder="2026 Fall" /></div>
            </div>
      ...
      <Dialog open={!!qrTeam} onOpenChange={(o) => !o && setQrTeam(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>{qrTeam?.name} — Registration QR</DialogTitle></DialogHeader>
...
function QRPanel({ team, onRegen }: { team: Team; onRegen: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">Athletes scan this to join <span className="font-medium text-foreground">{team.name}</span>. Coach approves each registration before it becomes active.</p>
      {isPrivatePreview && (
        <div className="rounded-md border bg-muted p-3 text-sm text-muted-foreground">
          This is a private preview link. Publish the app, then open Teams from the published app before sharing QR codes with athletes.
        </div>
      )}
      ...
      <div className="flex justify-between">
        <Button size="sm" variant="ghost" onClick={onRegen}><RefreshCw className="h-4 w-4" /> Regenerate link</Button>
        <a className="text-sm underline" href={url} target="_blank" rel="noreferrer">Open form</a>
      </div>
    </div>
  );
}
...
function PendingInbox({ registrations, teams }: { registrations: Registration[]; teams: Team[] }) {
  ...
  const approve = useMutation({
    mutationFn: async (r: Registration) => {
      ...
      const { data: inserted, error: aerr } = await supabase.from("athletes").insert({
        name: displayName,
        ...
        team_id: r.team_id,
        organization_id: (r as never as { organization_id: string | null }).organization_id ?? team?.organization_id ?? null,
        status: "active",
      } as never).select("id, organization_id").single();
      if (aerr) throw aerr;

      // Auto-enroll into every team in the same org whose sport matches a seasonal pick
      const orgId = (inserted as { organization_id: string | null } | null)?.organization_id ?? team?.organization_id ?? null;
      ...
      if (athleteId && orgId) {
        ...
        if (wanted.length > 0) {
          const { data: matchTeams } = await supabase
            ...
            .is("archived_at", null)
            .in("sport", wanted.map((w) => w.sport as string));
        ...
        if (rows.length > 0) {
          await supabase.from("athlete_teams").upsert(rows as never, { onConflict: "athlete_id,team_id" });
        }
      }

      const { error: rerr } = await supabase.from("registrations")
  ...
  const reject = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["registrations"] }); toast.info("Registration rejected"); },
  });

  if (!pending.length) return null;
  return (
    <Card>
      <CardHeader><CardTitle className="text-base">Pending registrations ({pending.length})</CardTitle></CardHeader>
      <CardContent className="space-y-2">
        {pending.map((r) => (
          <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-3">
            <div className="min-w-0">
              <div className="font-medium">{r.first_name} {r.last_name}{r.preferred_name ? ` (${r.preferred_name})` : ""}</div>
              <div className="text-xs text-muted-foreground">
                {teamMap.get(r.team_id)?.name ?? "—"} · Grade {r.grade ?? "—"} · Class of {r.graduation_year ?? "—"} · {r.weight_lb ?? "—"} lb
              </div>
              <div className="text-xs text-muted-foreground">
                Sports: {[
                  (r as never as { sport_fall?: string }).sport_fall && `Fall ${(r as never as { sport_fall: string }).sport_fall}`,
                  (r as never as { sport_winter?: string }).sport_winter && `Winter ${(r as never as { sport_winter: string }).sport_winter}`,
                  (r as never as { sport_spring?: string }).sport_spring && `Spring ${(r as never as { sport_spring: string }).sport_spring}`,
                ].filter(Boolean).join(" · ") || r.sport || "—"}
                {(r as never as { date_of_birth?: string }).date_of_birth ? ` · DOB ${(r as never as { date_of_birth: string }).date_of_birth}` : ""}
              </div>
              {(r.athlete_email || r.parent_email) && (
                <div className="text-xs text-muted-foreground">{[r.athlete_email, r.parent_email].filter(Boolean).join(" · ")}</div>
              )}
            </div>
