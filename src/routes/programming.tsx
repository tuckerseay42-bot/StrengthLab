  ...
  head: () => ({
    meta: [
      { title: "Programming Workspace — Strength Lab" },
      {
        name: "description",
        content:
          "One-screen program builder: hierarchy tree on the left, contextual editor on the right.",
      },
    ],
  }),
  ...
  const { data: allSessionsForResolve = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("program_sessions" as never)
        .select("*")
        .eq("workout_id", sel.id)
        .limit(1);
        ...
        <Sheet open={treeOpen} onOpenChange={setTreeOpen}>
          <SheetTrigger asChild>
            <Button size="sm" variant="outline" className="h-8">
              <PanelLeft className="h-4 w-4" /> Programs
            </Button>
          </SheetTrigger>
...
function Breadcrumb({
  ...
  const { data: phases = [] } = useQuery({
    ...programPhasesQO(programId || ""),
    enabled: !!programId,
  });
  const { data: cycles = [] } = useQuery({
    ...programCyclesQO(programId || ""),
    enabled: !!programId,
  });
  const { data: sessions = [] } = useQuery({
    ...programSessionsQO(programId || ""),
    enabled: !!programId,
  });
  ...
  if (sel.kind === "none" || sel.kind === "workout") {
    return (
      <div
        className={cn(
          "flex items-center border-b px-4 py-1.5 text-xs text-muted-foreground",
          compact && "border-none px-0",
        )}
      >
  ...
  const phaseId =
    sel.kind === "phase"
      ? sel.id
      : sel.kind === "cycle"
        ? cycles.find((c) => c.id === sel.id)?.phase_id
        : sel.kind === "session"
          ? sessions.find((s) => s.id === sel.id)?.phase_id
          : null;
  const cycleId =
    sel.kind === "cycle"
      ? sel.id
      : sel.kind === "session"
        ? sessions.find((s) => s.id === sel.id)?.cycle_id
        : null;
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-0.5 border-b px-3 py-1.5",
        compact && "border-none px-0 py-0",
      )}
    >
      {program && (
        <Crumb
      ...
      {session && (
        <>
          <Sep />
          <Crumb label={session.name} active />
        </>
      )}
    </div>
  );
...
function ContextNewButton({
  ...
  const addCycle = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase
        .from("program_cycles" as never)
        .insert({
          phase_id: phase.id,
          program_id: program.id,
          organization_id: program.organization_id,
          name: `Week ${phaseCycles.length + 1}`,
          weeks: 1,
          position: phaseCycles.length,
        } as never)
        .select("id")
        .single();
      if (error) throw error;
...
function ProgramNode({
  ...
  const handlePhaseDragEnd = (evt: DragEndEvent) => {
    ...
    qc.setQueryData(["program_phases", program.id], () =>
  ...
  return (
    <li>
      <Row
        ...
        menu={
          <>
            <DropdownMenuItem onClick={() => addPhase.mutate()}>
...
function PhaseNode({
    ...
    ((sel.kind === "cycle" || sel.kind === "session") &&
      cycles.some(
        (c) =>
          ...
          (sel.kind === "cycle"
            ? sel.id
            : sessions.find((s) => s.id === (sel as { id: string }).id)?.cycle_id),
      ));
  const [open, setOpen] = useState(contains);
  useEffect(() => {
    if (contains) setOpen(true);
  ...
  const addCycle = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("program_cycles" as never).insert({
        ...
        program_id: program.id,
        organization_id: program.organization_id,
        name: `Week ${cycles.length + 1}`,
        weeks: 1,
        position: cycles.length,
      } as never);
      if (error) throw error;
    },
  ...
  const handleDragEnd = (evt: DragEndEvent) => {
    ...
    qc.setQueryData(["program_cycles", program.id], (prev: ProgramCycle[] | undefined) => {
      ...
      return [...others, ...next.map((c, i) => ({ ...c, position: i }))].sort(
  ...
  return (
    <li>
      <Row
        ...
        menu={
          <>
            <DropdownMenuItem onClick={() => addCycle.mutate()}>
              <FolderPlus className="mr-2 h-3.5 w-3.5" />
              Add cycle / week
            </DropdownMenuItem>
            <DropdownMenuSeparator />
    ...
    mutationFn: async (ordered: ProgramSession[]) => {
      await Promise.all(
        ordered.map((s, i) =>
          supabase
            .from("program_sessions" as never)
            .update({ position: i, day: i + 1 } as never)
            .eq("id", s.id),
        ),
      );
    },
  ...
  const handleDragEnd = (evt: DragEndEvent) => {
    ...
    qc.setQueryData(["program_sessions", program.id], (prev: ProgramSession[] | undefined) => {
      ...
      return [...others, ...updated].sort(
        (x, y) => x.week - y.week || x.day - y.day || x.position - y.position,
      );
    });
    reorder.mutate(next);
  };
  ...
  return (
    <li>
      <Row
        ...
        menu={
          <>
            <DropdownMenuItem onClick={() => addSession.mutate()}>
  ...
  return (
    <li>
      <Row
        ...
        menu={
          <>
            <DropdownMenuItem onClick={() => setPickerOpen("copy")}>
              <Copy className="mr-2 h-3.5 w-3.5" />
              Copy to…
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setPickerOpen("move")}>
...
function DestinationPicker({
  ...
  const [activeOrgId] = useActiveOrgId();
  const { data: programsAll = [] } = useQuery(programsQO);
  const programs = activeOrgId
    ? programsAll.filter((p) => p.organization_id === activeOrgId)
    : programsAll;
  ...
  const run = useMutation({
      ...
      if (mode === "move") {
        ...
        for (const c of cycles) {
          const { data: existing } = await supabase
            ...
            .limit(1);
          const nextPos =
            ((existing?.[0] as { position?: number } | undefined)?.position ?? -1) + 1;
    ...
    onSuccess: (programIds) => {
      new Set(programIds).forEach((pid) =>
        qc.invalidateQueries({ queryKey: ["program_sessions", pid] }),
      );
      toast.success(
        mode === "move"
          ? "Session moved"
          : `Copied to ${selected.size} destination${selected.size > 1 ? "s" : ""}`,
      );
      onClose();
    },
    onError: (e: Error) => toast.error(toUserMessage(e)),
          ...
          <ul className="divide-y">
            {programs.map((p) => (
              <ProgramTargetGroup
                ...
                onToggle={toggle}
                mode={mode}
                currentCycleId={session.cycle_id}
              />
            ))}
          </ul>
        </div>
  ...
  return (
    <li className="p-1">
      <button
        ...
        className="flex w-full items-center gap-1 rounded px-1 py-1 text-left text-xs font-semibold hover:bg-muted"
      >
        {open ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
...
function CycleDestinationPicker({
  cycleId,
  sessions,
  onClose,
}: {
  ...
  const run = useMutation({
    ...
    onSuccess: (programIds) => {
      ...
      toast.success(
        `Copied ${sessions.length} session${sessions.length === 1 ? "" : "s"} to ${selected.size} cycle${selected.size === 1 ? "" : "s"}`,
      );
      onClose();
    },
    onError: (e: Error) => toast.error(toUserMessage(e)),
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Copies all {sessions.length} session{sessions.length === 1 ? "" : "s"} in this cycle
            into one or more destination cycles/weeks. Sessions point at the same workouts (not
            duplicated).
          </DialogDescription>
        </DialogHeader>
        <div className="max-h-[50vh] overflow-auto rounded border">
...
function EditorPane({
  ...
  if (sel.kind === "phase")
    return <PhaseEditor phaseId={sel.id} programId={sel.programId} onSelect={onSelect} />;
  if (sel.kind === "cycle")
    return <CycleEditor cycleId={sel.id} programId={sel.programId} onSelect={onSelect} />;
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently removes “{workout.name}”, including its exercises, sets, and
              assignments. Completed logs are preserved.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function ProgramEditor({ programId, onSelect }: { programId: string; onSelect: (s: Sel) => void }) {
  ...
  const setAdvanced = (v: boolean) => {
    setAdvancedState(v);
    try {
      window.localStorage.setItem(advancedKey, String(v));
    } catch {
      /* noop */
    }
  };
  ...
  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("programs" as never)
        .update({
          name: form.name.trim() || program?.name,
  ...
  const addDay = useMutation({
    mutationFn: async () => {
      if (!defaultCycle || !defaultPhase || !program)
        throw new Error("Missing default phase/cycle");
  ...
  const saveVersion = useMutation({
    mutationFn: async () => {
      ...
      const label = prompt(
        'Label this version (e.g. "Week 3 draft", "Pre-deload")',
    ...
    onError: (e: Error) => {
      if (e.message !== "__cancelled") toast.error(toUserMessage(e));
    },
  });
  ...
  const deleteVersion = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase
        .from("program_versions" as never)
        .delete()
        .eq("id", id);
      if (error) throw error;
    },
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {c.length} cycles · {s.length} sessions
                    </div>
                  </button>
                );
              })}
          ...
          <div className="mt-4">
            ...
            {versions.length === 0 ? (
              <div className="rounded-md border border-dashed p-4 text-center text-xs text-muted-foreground">
                No snapshots yet — save one before making a big change so you can look back at how
        ...
        <>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <Button
              ...
              disabled={addDay.isPending || !defaultCycle}
            >
            ...
            <Card className="mt-3 border-dashed">
              <CardContent className="grid gap-3 p-8 text-center">
                ...
                <p className="mx-auto max-w-sm text-sm text-muted-foreground">
                  Add a training day and start programming exercises right away. No phases, cycles,
                  or velocity targets required — flip on Advanced programming above when you want
                  the full periodization model.
                </p>
        ...
        <CardContent className="grid gap-3 p-4 md:grid-cols-5">
          ...
          <div className="md:col-span-5">
            ...
            <Input
              ...
              placeholder="Peak strength / speed / GPP…"
            />
          </div>
...
function SessionEditor({ sessionId, programId }: { sessionId: string; programId: string }) {
  ...
  const workout: Workout | undefined = session?.workout_id
    ? workouts.find((w) => w.id === session.workout_id)
    : undefined;
  const { data: programs = [] } = useQuery(programsQO);
  ...
  const rename = useMutation({
    mutationFn: async () => {
      ...
      if (session.workout_id) {
        await supabase
          .from("workouts")
          .update({ name: name.trim() || session.name } as never)
  ...
  const duplicateSession = useMutation({
    ...
    onSuccess: (newId) => {
      ...
      navigate({
        to: "/programming",
        search: selToSearch({ kind: "session", id: newId, programId }),
        replace: false,
      });
    },
    onError: (e: Error) => toast.error(toUserMessage(e)),
          ...
          programContext={
            program
              ? {
                  ...
                  teamName: program.team_id
                    ? (teams.find((t) => t.id === program.team_id)?.name ?? null)
                    : null,
      ...
      <AlertDialog open={confirmDelWorkout} onOpenChange={setConfirmDelWorkout}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this workout?</AlertDialogTitle>
            <AlertDialogDescription>
              Removes the workout, its exercises, sets, and assignments. The session stays and you
              can attach a new workout to it. Completed logs are preserved.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
...
function NewProgramDialog({
  ...
  const qc = useQueryClient();
  const [form, setForm] = useState({ name: "", description: "", team_id: "__none" });
  const create = useMutation({
    mutationFn: async () => {
      if (!form.name.trim()) throw new Error("Name required");
      const teamId = form.team_id === "__none" ? null : form.team_id;
      const orgId = teamId
        ? (teams.find((t) => t.id === teamId)?.organization_id ?? (await getScopedOrgId()))
        : await getScopedOrgId();
      const { data, error } = await supabase
        .from("programs" as never)
        .insert({
          name: form.name.trim(),
      ...
      try {
        window.localStorage.setItem(`sl.program.advanced.${programId}`, "false");
      } catch {
        /* noop */
      }
      return programId;
    ...
    onSuccess: (id) => {
      ...
      setForm({ name: "", description: "", team_id: "__none" });
      onOpenChange(false);
      onCreated(id);
    },
    onError: (e: Error) => toast.error(toUserMessage(e)),
        ...
        <div className="grid gap-3">
          ...
          <div>
            ...
            <Select value={form.team_id} onValueChange={(v) => setForm({ ...form, team_id: v })}>
              ...
              <SelectContent>
                <SelectItem value="__none">No team (assign later)</SelectItem>
                {teams.map((t) => (
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Programs don't have to belong to a team — assign athletes to this program from Teams
              or Athletes.
            </p>
          </div>
        </div>
