...
export function AthleteSpiderGraph({ athlete }: { athlete: Athlete }) {
  ...
  const effectiveGroup: SpiderComparisonGroup | null =
    groupOverride ?? template?.comparison_group ?? null;
  ...
  const visible = rows.filter(
    (r) => !(r.missing && templateMetrics.find((m) => m.metric_key === r.key)?.hide_if_missing),
  );
      ...
      <CardContent className="p-4 sm:p-5">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3 sm:flex sm:items-baseline sm:justify-between">
          <div className="min-w-0">
            <div className="eyebrow" style={{ color: "oklch(0.82 0.17 85)" }}>
              Physicality percentiles
            </div>
              ...
              >
                <defs>
                  <radialGradient id="spiderFill" cx="50%" cy="50%" r="70%">
                    <stop offset="0%" stopColor="oklch(0.82 0.17 85)" stopOpacity={0.1} />
                ...
                <Tooltip
                  contentStyle={{
                    ...
                    border:
                      "1px solid color-mix(in oklab, oklch(0.82 0.17 85) 40%, var(--color-border))",
                    borderRadius: 8,
                  ...
                  formatter={(
                    v: number,
                    name: string,
                    entry: { payload?: { _raw?: number; _unit?: string; _missing?: boolean } },
                  ) => {
                    const p = entry.payload ?? {};
                    if (name === "Athlete") {
                      if (p._missing) return ["No data", "Athlete"];
                      return [
                        `${v} / 100${p._raw != null ? ` · ${p._raw.toFixed(2)}${p._unit ?? ""}` : ""}`,
                        "Athlete",
                      ];
                    }
                    return [`${v} / 100`, name];
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-3 text-[10px] uppercase tracking-widest text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1">
              <span
                className="h-2 w-2 rounded-full"
                style={{ background: "oklch(0.82 0.17 95)" }}
              />{" "}
              Mid
            </span>
            <span className="inline-flex items-center gap-1">
              <span
                className="h-2 w-2 rounded-full"
                style={{ background: "oklch(0.86 0.05 300)" }}
              />{" "}
              Peer
            </span>
          </div>
