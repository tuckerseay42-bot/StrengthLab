...
import type {
  Athlete, TestRow, LiftRow, CustomMetric, CustomTestType,
  SpiderTemplate, SpiderMetric, SpiderComparisonGroup, SpiderDateRule,
} from "@/lib/queries";
...
function bestTestPerAthlete(
  tests: TestRow[], testType: string, lowerIsBetter: boolean, cutoff: string | null,
): Map<string, number> {
...
function comparisonPool(
  athletes: Athlete[], athlete: Athlete, group: SpiderComparisonGroup,
): Athlete[] {
  ...
  switch (group) {
    ...
    case "sport":
      return athlete.sport ? athletes.filter((a) => norm(a.sport) === norm(athlete.sport)) : athletes;
    case "grade":
      return athlete.grade != null ? athletes.filter((a) => a.grade === athlete.grade) : athletes;
    case "position":
      return athlete.position
        ? athletes.filter((a) => norm(a.position) === norm(athlete.position) && norm(a.sport) === norm(athlete.sport))
        : athletes;
    case "org":
    case "custom":
    default:
...
export function computeSpider(input: SpiderInput): SpiderRow[] {
  const { template, metrics, athlete, athletes, tests, lifts, customMetrics, customTypes } = input;
  ...
  for (const m of metrics.slice().sort((a, b) => a.position - b.position)) {
    ...
    if (kind === "test") {
      ...
      if (!cm) {
        rows.push({ key: m.metric_key, label, unit: "", raw: null, normalized: null, groupMedian: null, missing: true, lowerIsBetter: false });
        continue;
      }
      label = m.display_label ?? cm.name;
      unit = cm.unit ?? "";
      lowerIsBetter = cm.lower_is_better;
      const results = computeMetric(cm, pool, tests, lifts, [], { from: cutoff ?? null, to: null }, customTypes);
      mine = results.find((r) => r.athlete.id === athlete.id)?.value ?? null;
      poolValues = results.map((r) => r.value).filter((v): v is number => v != null && Number.isFinite(v));
    } else {
      rows.push({ key: m.metric_key, label, unit: "", raw: null, normalized: null, groupMedian: null, missing: true, lowerIsBetter: false });
      continue;
    }
    ...
    if (mine == null) {
      rows.push({ key: m.metric_key, label, unit, raw: null, normalized: null, groupMedian: null, missing: true, lowerIsBetter });
      continue;
    }
    ...
    switch (template.normalization_method) {
      ...
      case "threshold": {
        ...
        normalized = best === 0 ? 0 : Math.round(Math.min(100, lowerIsBetter ? (best / mine) * 100 : (mine / best) * 100));
        break;
      }
      case "percentile":
      default:
...
export function pickTemplate(
  templates: SpiderTemplate[], athlete: Athlete,
): SpiderTemplate | null {
